...
CasterSwerveDriveController::CasterSwerveDriveController()
    : controller_interface::ControllerInterface(), ref_timeout_(0, 0) {}
// *****************************************
...
CallbackReturn CasterSwerveDriveController::on_init() {
  ...
  try {
    ...
    auto_declare<std::vector<double>>("module_x_offsets",
                                      std::vector<double>{});
    auto_declare<std::vector<double>>("module_y_offsets",
                                      std::vector<double>{});
    auto_declare<std::vector<double>>("module_angle_offsets",
                                      std::vector<double>{});
    auto_declare<std::vector<double>>("module_steering_limit_lower",
                                      std::vector<double>{});
    auto_declare<std::vector<double>>("module_steering_limit_upper",
                                      std::vector<double>{});
    auto_declare<std::vector<double>>("module_wheel_speed_limit_lower",
                                      std::vector<double>{});
    auto_declare<std::vector<double>>("module_wheel_speed_limit_upper",
                                      std::vector<double>{});
    ...
    auto_declare<std::string>("visualization_marker_topic",
                              "~/swerve_visualization_markers");
...
CasterSwerveDriveController::command_interface_configuration() const {
  ...
  try {
    ...
    steering_names =
        get_node()->get_parameter("steering_joint_names").as_string_array();
    wheel_names =
        get_node()->get_parameter("wheel_joint_names").as_string_array();
  } catch (const std::exception &e) {
    // Log error but don't crash, configuration might be incomplete
    RCLCPP_ERROR(get_node()->get_logger(),
                 "Error reading joint names during command config: %s.",
                 e.what());
    // It's safer to return an empty config here, let CM handle missing
    // interfaces later
    return conf;
  }
...
CallbackReturn CasterSwerveDriveController::on_configure(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  auto logger = get_node()->get_logger();
  RCLCPP_DEBUG(logger, "Configuring Caster Swerve Drive Controller...");
  ...
  // Get parameters
  try {
    steering_joint_names_ =
        get_node()->get_parameter("steering_joint_names").as_string_array();
    wheel_joint_names_ =
        get_node()->get_parameter("wheel_joint_names").as_string_array();
    wheel_radius_ = get_node()->get_parameter("wheel_radius").as_double();
    module_x_offsets_ =
        get_node()->get_parameter("module_x_offsets").as_double_array();
    module_y_offsets_ =
        get_node()->get_parameter("module_y_offsets").as_double_array();
    module_angle_offsets_ =
        get_node()->get_parameter("module_angle_offsets").as_double_array();
    module_steering_limit_lower_ =
        get_node()
            ->get_parameter("module_steering_limit_lower")
            .as_double_array();
    module_steering_limit_upper_ =
        get_node()
            ->get_parameter("module_steering_limit_upper")
            .as_double_array();
    module_wheel_speed_limit_lower_ =
        get_node()
            ->get_parameter("module_wheel_speed_limit_lower")
            .as_double_array();
    module_wheel_speed_limit_upper_ =
        get_node()
            ->get_parameter("module_wheel_speed_limit_upper")
            .as_double_array();
    steering_angular_velocity_limit_ =
        get_node()
            ->get_parameter("steering_angular_velocity_limit")
            .as_double();
    enabled_steering_flip_ =
        get_node()->get_parameter("enabled_steering_flip").as_bool();
    enabled_open_loop_ =
        get_node()->get_parameter("enabled_open_loop").as_bool();
    enabled_steering_angular_velocity_limit_ =
        get_node()
            ->get_parameter("enabled_steering_angular_velocity_limit")
            .as_bool();
    ...
    steering_alignment_angle_error_threshold_ =
        get_node()
            ->get_parameter("steering_alignment_angle_error_threshold")
            .as_double();
    steering_alignment_start_angle_error_threshold_ =
        get_node()
            ->get_parameter("steering_alignment_start_angle_error_threshold")
            .as_double();
    steering_alignment_start_speed_error_threshold_ =
        get_node()
            ->get_parameter("steering_alignment_start_speed_error_threshold")
            .as_double();

    cmd_vel_topic_ = get_node()->get_parameter("cmd_vel_topic").as_string();
    use_stamped_cmd_vel_ =
        get_node()->get_parameter("use_stamped_cmd_vel").as_bool();
    ...
    twist_covariance_diagonal_ =
        get_node()
            ...
            .as_double_array();
    odom_solver_method_str_ =
        get_node()->get_parameter("odom_solver_method").as_string();
    odom_source_ = get_node()->get_parameter("odom_source").as_string();
    enable_visualization_ =
        get_node()->get_parameter("enable_visualization").as_bool();
    ...
    limiter_linear_x_ = SpeedLimiter(
        ...
        params_.linear.x.max_acceleration, params_.linear.x.min_jerk,
    ...
    limiter_linear_y_ = SpeedLimiter(
        ...
        params_.linear.y.max_acceleration, params_.linear.y.min_jerk,
  ...
  if (steering_joint_names_.size() != num_modules_ ||
      wheel_joint_names_.size() != num_modules_ ||
      module_x_offsets_.size() != num_modules_ ||
      module_y_offsets_.size() != num_modules_ ||
    ...
    RCLCPP_FATAL(logger,
                 "Parameter array lengths do not match expected number of "
                 "modules (%ld).",
                 num_modules_);
    RCLCPP_FATAL(
        logger,
        "steering_joint_names: %zu, wheel_joint_names: %zu,"
        " module_x_offsets: %zu, module_y_offsets: %zu, module_angle_offsets: "
        "%zu,"
        " module_steering_limit_lower: %zu, module_steering_limit_upper: %zu, "
        "module_wheel_speed_limit_lower: %zu, module_wheel_speed_limit_upper: "
        "%zu",
        ...
        module_wheel_speed_limit_lower_.size(),
  ...
  cmd_vel_subscriber_ = get_node()->create_subscription<CmdVelMsg>(
      ...
      std::bind(&CasterSwerveDriveController::reference_callback, this,
                std::placeholders::_1));
  ...
  RCLCPP_DEBUG(logger, "Subscribed to %s (expecting geometry_msgs/msg/Twist)",
  ...
  try {
    ...
    odometry_.setModuleParams(module_x_offsets_, module_y_offsets_,
                              wheel_radius_);
  ...
  rt_commanded_joint_state_publisher_ =
      std::make_unique<CommandedJointStatePublisher>(
          commanded_joint_state_publisher_);
  ...
  if (rt_commanded_joint_state_publisher_) {
    ...
    msg.position.resize(num_modules_ * 2,
                        std::numeric_limits<double>::quiet_NaN());
    msg.velocity.resize(num_modules_ * 2,
                        std::numeric_limits<double>::quiet_NaN());
  ...
  if (enable_visualization_) {
    ...
    if (visualization_update_time_ < 0.0) {
      RCLCPP_WARN(logger,
                  "visualization_update_time (%.3f) is negative. Setting to "
                  "0.0 (publish every update).",
                  visualization_update_time_);
    ...
    if (get_node()) {
      last_visualization_publish_time_ =
          get_node()->now() -
          rclcpp::Duration::from_seconds(visualization_update_time_ + 1.0);
    } else {
      // no initialization, set to 0
      last_visualization_publish_time_ = rclcpp::Time(0, 0, RCL_ROS_TIME);
    }

    visualizer_->init(this->get_node(), base_frame_id_,
                      visualization_marker_topic_, num_modules_,
                      visualization_update_time_);
  }

  // ---publish_limited_velocity  ---
  if (publish_limited_velocity_) {
    limited_velocity_publisher_ = get_node()->create_publisher<Twist>(
        "limited_cmd_vel", rclcpp::SystemDefaultsQoS());
...
CallbackReturn CasterSwerveDriveController::on_activate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  RCLCPP_DEBUG(get_node()->get_logger(),
               "Activating Swerve Drive Controller...");
  ...
  RCLCPP_DEBUG(get_node()->get_logger(), "Attempting to claim %zu modules.",
               num_modules_);
  RCLCPP_DEBUG(get_node()->get_logger(), "Available command interfaces (%zu):",
               command_interfaces_.size());
  RCLCPP_DEBUG(get_node()->get_logger(),
               "Available state interfaces (%zu):", state_interfaces_.size());

  for (size_t i = 0; i < num_modules_; ++i) {
    // ***** find the steering and wheel joint names according to the module
    // index *****
    const auto &steering_joint = steering_joint_names_[i];
    const auto &wheel_joint = wheel_joint_names_[i];
    RCLCPP_DEBUG(get_node()->get_logger(),
                 "Processing module %zu:"
                 " Expected Steering='%s', Expected Wheel='%s'",
                 i, steering_joint.c_str(), wheel_joint.c_str());
    // **********************************

    // --- Find state interface for Steering Position
    const hardware_interface::LoanedStateInterface *steering_state_pos_ptr =
        nullptr;
    ...
    const hardware_interface::LoanedStateInterface *wheel_state_vel_ptr =
        nullptr;

    RCLCPP_DEBUG(get_node()->get_logger(),
                 "Searching for State Interface: Joint='%s', Type='%s'",
                 expected_steering_state_name.c_str(),
                 expected_steering_state_if_name.c_str());
    bool state_found = false;
    const std::string expected_position_state_name =
        steering_joint_names_[i] + "/" + HW_IF_POSITION;
    const std::string expected_speed_state_name =
        wheel_joint_names_[i] + "/" + HW_IF_VELOCITY;
    ...
    if (!state_found) {
      RCLCPP_ERROR_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(),
                            1000,
                            "State interface '%s' not found in "
                            "state_interfaces_ list during update.",
    ...
    RCLCPP_DEBUG(get_node()->get_logger(),
                 "  Searching for Command Interface: Joint='%s', Type='%s'",
                 expected_steering_cmd_name.c_str(),
                 expected_steering_cmd_if_name.c_str());
    ...
    if (!cmd_steering_found) {
      RCLCPP_ERROR_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(),
                            1000,
                            "Command interface '%s' not found in "
                            "command_interfaces_ list during update.",
                            expected_steering_cmd_name.c_str());
      return CallbackReturn::ERROR;
    }
    ...
    const std::string expected_wheel_cmd_name =
        wheel_joint_names_[i] + "/" + HW_IF_VELOCITY;
    ...
    RCLCPP_DEBUG(get_node()->get_logger(),
                 ...
                 expected_wheel_cmd_if_name.c_str());
    ...
    try {
      module_handles_.emplace_back(ModuleHandles{
          std::cref(*steering_state_pos_ptr), std::ref(*steering_cmd_pos_ptr),
      ...
      RCLCPP_DEBUG(get_node()->get_logger(),
                   "Successfully processed interfaces for module %zu.", i);
    } catch (const std::exception &e) {
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Exception while adding module handles for module %zu: %s",
                   i, e.what());
      module_handles_.clear();
      return CallbackReturn::ERROR;
    }
...
CallbackReturn CasterSwerveDriveController::on_deactivate(
    const rclcpp_lifecycle::State & /*previous_state*/) {
  RCLCPP_INFO(get_node()->get_logger(),
              "Deactivating Swerve Drive Controller...");
  // Stop the robot
  for (size_t i = 0; i < num_modules_; ++i) {
    try {
      ...
      if (!module_handles_.empty() && i < module_handles_.size()) {
        ...
        auto get_steering_val =
            module_handles_[i].steering_state_pos.get().get_optional();
        if (!module_handles_[i].steering_cmd_pos.get().set_value(
                get_steering_val.value())) {
          RCLCPP_WARN(
        ...
        for (auto &iface : command_interfaces_) {
          // Check if wheel_joint_names_ is still valid and index is within
          // bounds
          if (i < wheel_joint_names_.size() &&
              iface.get_name() == wheel_joint_names_[i] &&
              iface.get_interface_name() == HW_IF_VELOCITY) {
            if (!iface.set_value(0.0)) {
              RCLCPP_WARN(get_node()->get_logger(),
                          "Failed to set value for interface %s",
      ...
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Error setting command interface values during deactivation "
                   "for module %zu: %s",
                   i, e.what());
    } catch (...) {
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Unknown error setting command interface values during "
                   "deactivation for module %zu",
                   i);
    }
  }
...
double CasterSwerveDriveController::normalize_angle(double angle_rad) {
...
double CasterSwerveDriveController::normalize_angle_positive(double angle) {
  // Use fmod and add 2*PI to handle negative results correctly
  return std::fmod(std::fmod(angle, 2.0 * M_PI) + 2.0 * M_PI, 2.0 * M_PI);
}

double CasterSwerveDriveController::shortest_angular_distance(double from,
                                                              double to) {
...
CasterSwerveDriveController::update(const rclcpp::Time &time,
  ...
  RCLCPP_DEBUG(get_node()->get_logger(), "Reading command velocity: %s",
               current_cmd_vel_ptr ? "valid" : "invalid");

  // check if the command velocity is valid
  bool timeout = false;
  // Check if ref_timeout_ is valid (non-zero duration) before calculating
  // difference
  if (ref_timeout_.seconds() > 0.0 &&
      (time - last_cmd_vel_time_) > ref_timeout_) {
  ...
  if (timeout) {
    ...
    if (target_vx_ != current_cmd_vel.linear.x ||
      ...
      RCLCPP_DEBUG(get_node()->get_logger(),
                   ...
                   current_cmd_vel.linear.x, current_cmd_vel.linear.y,
  ...
  if (std::isnan(target_vx_) || std::isnan(target_vy_) ||
      std::isnan(target_wz_)) {
    RCLCPP_DEBUG_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(),
                          5000,
                          "Received NaN in target velocity (vx:%.2f, vy:%.2f, "
                          "wz:%.2f). Setting targets to zero.",
  ...
  // --- 1.1 command may be limited further by SpeedLimit without affecting the
  // stored twist command
  if (enabled_speed_limits_) {
    ...
    // target_vx_, target_vy_, target_wz_ is the current command velocity
    // (before limiting) SpeedLimiter limits the target velocities based on the
    // previous command and the time period
    limiter_linear_x_.limit(target_vx_, previous_cmd.linear.x,
                            pprevious_cmd.linear.x, time_gap);
    limiter_linear_x_.limit(target_vy_, previous_cmd.linear.y,
                            pprevious_cmd.linear.y, time_gap);
    limiter_angular_z_.limit(target_wz_, previous_cmd.angular.z,
                             pprevious_cmd.angular.z, time_gap);
  ...
  RCLCPP_DEBUG_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(),
                        1000, "Number of modules: %zu", num_modules_);
  for (size_t i = 0; i < num_modules_; ++i) {
    RCLCPP_DEBUG(get_node()->get_logger(), "Reading state for module %zu", i);
    try {
      // validate module_handles_ before accessing it
      RCLCPP_DEBUG(get_node()->get_logger(), "Module handles size: %zu",
                   module_handles_.size());
      if (module_handles_.empty() || i >= module_handles_.size()) {
        RCLCPP_ERROR_THROTTLE(
            get_node()->get_logger(), *get_node()->get_clock(), 1000,
            "Module handles not ready for index %zu in state reading.", i);
        all_states_read = false;
        break;
      }
      auto cur_steering_get_val =
          module_handles_[i].steering_state_pos.get().get_optional();
      auto cur_wheel_get_val =
          module_handles_[i].wheel_state_vel.get().get_optional();
      ...
      corrected_steering_positions.push_back(current_steering_positions +
                                             module_handles_[i].angle_offset);
      current_wheel_velocities.push_back(cur_wheel_get_val.value());
    } catch (const std::exception &e) {
      RCLCPP_ERROR_THROTTLE(
          get_node()->get_logger(), *get_node()->get_clock(), 1000,
          "Exception reading state for module %zu during odometry update: %s",
          i, e.what());
      all_states_read = false;
      break;
    }
  ...
  if (all_states_read) {
    ...
    if (odom_source_ == "command") {
      // update the odometry using just target velocities(command), it will be
      // not good
      if (!odometry_.update(target_vx_, target_vy_, target_wz_, dt)) {
        RCLCPP_WARN_THROTTLE(
            get_node()->get_logger(), *get_node()->get_clock(), 1000,
            "Odometry update failed, dt might be too small (%.6f).", dt);
      }
    } else if (odom_source_ == "feedback") {
      // calcuate the odometry using the kinematics from the steering and wheel
      // velocities
      if (!odometry_.update(corrected_steering_positions,
                            current_wheel_velocities, dt)) {
        RCLCPP_WARN_THROTTLE(
            get_node()->get_logger(), *get_node()->get_clock(), 1000,
            "Odometry update failed, dt might be too small (%.6f).", dt);
      }
      ...
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Invalid odometry source selected.");
    }

  } else {
    RCLCPP_WARN_THROTTLE(
        get_node()->get_logger(), *get_node()->get_clock(), 1000,
        "Skipping odometry update due to state reading errors.");
  }
  RCLCPP_DEBUG(get_node()->get_logger(),
               "Computed odometry: x=%.2f, y=%.2f, theta=%.2f ",
               odometry_.getX(), odometry_.getY(), odometry_.getYaw());

  // --- 4. calculate the wheel velocities and steering angles based on the
  // inverse kinematics ---
  ...
  for (size_t i = 0; i < num_modules_; ++i) {
    ...
    if (module_handles_.empty() || i >= module_handles_.size()) {
      RCLCPP_ERROR_THROTTLE(
          get_node()->get_logger(), *get_node()->get_clock(), 1000,
          "Module handles not initialized correctly for index %zu", i);
      continue;
    }
    ...
    double target_steering_angle_robot =
        std::atan2(wheel_vel_y, wheel_vel_x + 1e-9);
    double target_wheel_speed =
        std::sqrt(wheel_vel_x * wheel_vel_x + wheel_vel_y * wheel_vel_y);
      ...
      if (std::fabs(angle_diff) > M_PI * 0.5) {
        if (normalize_angle(target_steering_joint_angle + M_PI) > limit_lower &&
          ...
          optimized_steering_angle =
              normalize_angle(target_steering_joint_angle + M_PI);
          wheel_rotation_direction = -1.0;
          RCLCPP_DEBUG(get_node()->get_logger(),
                       "Module %zu: Flipping steering angle from %.2f to %.2f "
                       "(angle diff %.2f rad).",
                       i, target_steering_joint_angle, optimized_steering_angle,
                       angle_diff);
        } else {
          RCLCPP_WARN(get_node()->get_logger(),
                      "Module %zu: Flipping steering angle would result in "
        ...
        if (target_steering_joint_angle < limit_lower ||
          ...
          if (normalize_angle(target_steering_joint_angle + M_PI) >
                  limit_lower &&
              normalize_angle(target_steering_joint_angle + M_PI) <
                  limit_upper) {
            // Flip the steering angle by adding M_PI
    ...
    } else if (limited_steering_cmd < limit_lower ||
               limited_steering_cmd > limit_upper) {
      // Normal case
      RCLCPP_ERROR(get_node()->get_logger(),
                   "Module %zu: Target steering angle %.2f outside limits "
                   "[%.2f, %.2f]. Stopping.",
    ...
    if (enabled_steering_angular_velocity_limit_) {
      if (steering_angular_velocity_limit_ <
              std::numeric_limits<double>::max() &&
          steering_angular_velocity_limit_ > 0.0) {
        double max_allowed_steering_change_this_dt =
            steering_angular_velocity_limit_ * time_gap;

        if (is_no_limitation) {
          // If no limitation, just use the optimized steering angle

          double absolute_current_steering_angle =
              std::fmod(current_steering_angle, 2.0 * M_PI);

          double desired_steering_change_rad = shortest_angular_distance(
              absolute_current_steering_angle, limited_steering_cmd);
          double actual_steering_change_this_dt = std::clamp(
              desired_steering_change_rad, -max_allowed_steering_change_this_dt,
              max_allowed_steering_change_this_dt);

          // If the desired steering change is larger than the maximum allowed
          // change,
          optimized_steering_angle =
    ...
    double final_wheel_vel_cmd =
        wheel_rotation_direction * target_wheel_speed / wheel_radius_;
    ...
    try {
      ...
      if (fabs(current_wheel_velocity) >=
          steering_alignment_start_speed_error_threshold_) {
        if (steering_alignment_angle_error_threshold_ <=
            fabs(shortest_angular_distance(current_steering_angle,
                                           limited_steering_cmd))) {
          // when current steering angle is not close to the target steering
          // angle, stop the wheel
      ...
      if (final_wheel_vel_cmd < module_wheel_speed_limit_lower_[i] ||
        ...
        if (enabled_wheel_saturation_scaling_) {
          ...
          RCLCPP_WARN(get_node()->get_logger(),
                      "min scale factor: %.2f, final_wheel_vel_cmd: %.2f, "
                      "clipped_wheel_vel_cmd: %.2f",
                      wheel_saturation_scale_factor_, final_wheel_vel_cmd,
                      clipped_wheel_vel_cmd);
        } else {
      ...
      RCLCPP_DEBUG(
          get_node()->get_logger(),
          "Calculated Module %zu: Steering command %.2f, Wheel command %.2f", i,
          optimized_steering_angle, final_wheel_vel_cmd);
  ...
  if (target_vx_ == 0.00 && target_vy_ == 0.00 && target_wz_ == 0.00) {
    RCLCPP_DEBUG(get_node()->get_logger(),
                 "Robot is halted. Commanding zero wheel velocity and holding "
                 "current steering.");
    for (size_t i = 0; i < num_modules_; ++i) {
      if (module_handles_.empty() || i >= module_handles_.size()) {
        continue;
      ...
      try {
        ...
        if (!module_handles_[i].steering_cmd_pos.get().set_value(
                current_steering_angle_for_hold)) {
          RCLCPP_WARN(
              get_node()->get_logger(), "Failed to set value for interface %s",
              module_handles_[i].steering_cmd_pos.get().get_name().c_str());
        }
        ...
        RCLCPP_ERROR_THROTTLE(
            get_node()->get_logger(), *get_node()->get_clock(), 1000,
            "Exception during halted command writing for module %zu: %s", i,
            e.what());
      }
    }
  } else {
    ...
    for (size_t i = 0; i < num_modules_; ++i) {
      RCLCPP_DEBUG(
          get_node()->get_logger(),
          "Command Module %zu: Steering command %.2f, Wheel command %.2f", i,
  ...
  // --- 6. publish odometry message and TF and joint commadns and marker
  // visualization---
  tf2::Quaternion orientation;
  orientation.setRPY(0.0, 0.0, odometry_.getYaw());
  if (all_states_read && rt_odom_state_publisher_ &&
      rt_odom_state_publisher_->trylock()) {
  ...
  if (rt_commanded_joint_state_publisher_ &&
      rt_commanded_joint_state_publisher_->trylock()) {
    auto &msg = rt_commanded_joint_state_publisher_->msg_;
  ...
  if (enable_visualization_ && visualizer_) {
    if ((time - last_visualization_publish_time_).seconds() >=
        visualization_update_time_) {
      ...
      for (size_t i = 0; i < num_modules_; ++i) {
        // set the steering angles and wheel velocities for visualization
        // considering the offsets
        if (i < final_steering_commands.size() &&
            i < module_angle_offsets_.size()) {
        ...
        if (i < final_wheel_velocity_commands.size()) {
          wheel_linear_vels_for_viz[i] =
              final_wheel_velocity_commands[i] * wheel_radius_;
        } else {
      ...
      if (robot_frame_steering_angles_for_viz.size() == num_modules_) {
        visualizer_->publish_markers(time, target_vx_, target_vy_, target_wz_,
        ...
        RCLCPP_WARN_THROTTLE(get_node()->get_logger(), *get_node()->get_clock(),
                             1000,
                             "command/steering vectors not ready or size "
                             "mismatch for publishing.");
      }
    }
  }
...
void CasterSwerveDriveController::reference_callback(
    const std::shared_ptr<CmdVelMsg> msg) {
  ...
  RCLCPP_DEBUG(get_node()->get_logger(),
               "Received new command: vx=%.2f, vy=%.2f, wz=%.2f", msg->linear.x,
